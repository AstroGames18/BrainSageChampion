...
namespace BizzyBeeGames
{
	public class SoundManager : SingletonComponent<SoundManager>
	{
		...
		private string[] bgmMusics = new string[9] { "WelcomeScreenBGM", "StartScreenBGM", "GameScreenBGM", "ChallengeLevelBGM", "MiniGameBGM", "LevelWinBGM", "LevelLoseBGM", "OutOfMovesBGM", "GiftScreenBGM" };
		#endregion
		...
		private void Start()
		{
			if (!PlayerPrefs.HasKey("toggle_sound")) { PlayerPrefs.SetInt("toggle_sound", 1); }
			if (!PlayerPrefs.HasKey("toggle_music")) { PlayerPrefs.SetInt("toggle_music", 1); }

            PlayScreenBGM("WelcomeScreenBGM");
        }
		...
		public void StopScreenBGM()
		{
			foreach (string bgm in bgmMusics)
			{
				Stop(bgm);
			}
		}
