...
namespace BizzyBeeGames
{
	public class ScreenManager : SingletonComponent<ScreenManager>
	{
		...
		public void Show(string screenId, System.Action action = null, bool force = false)
		{
			...
			Show(screenId, false, false, action);
		}
		public void Hide(string screenId, bool back = false, bool immediate = true)
		{
			// Get the screen we want to show
			Screen screen = GetScreenById(screenId);
			screen.Hide(back, immediate);
		}
		...
		private void Show(string screenId, bool back, bool immediate, System.Action action = null)
		{
			...
			// Show the new screen
			screen.Show(back, immediate, action);
